...
| 17 | §4.4 改为首页；推荐组件去重 | §1.2、§4.4、§4.7.9–12、§4.18、§7.1、附录 C | 重构（v1.2.3） |
...
| 2026/09/23 | v1.2.3 | — | ⑰ §4.4 改为首页（Banner+一级分类+特色馆+Recommended）；全局推荐组件移至 §4.18 |
...
## 17. §4.4 首页结构 + 推荐组件去重（v1.2.3）

> **问题：** 原 §4.4「Recommended 推荐组件」与 §4.7.9–4.7.12 内容重复。  
> **处理：** §4.4 改为**首页核心展示区（#6）**；全局 Recommended（#18）**仅保留 §4.18 一处**。
...
4. Recommended 规则见 §4.18；与 Sort By recommend 枚举区分。
...
### §4.18 新增（粘贴原 §4.4 Recommended 精简版 + 删除 §4.7.9–4.7.12）

**飞书操作：**

1. **删除** 原 §4.4「模块 H：Recommended 推荐组件（#18）」整节  
2. **删除** §4.7.9–§4.7.12「商品推荐组件」重复节（保留 §4.7.11 通用商品卡字段）  
3. **新增 §4.18**「模块 H：全局推荐组件 Recommended（#18）」——内容见 `kickbazar-toc-web-prd.md` §4.18 或下方摘要：

```
Recommended 全站复用；displayMode=recommended 随机序。
宿主：home（View More）/ pdp / cart / search_empty / store / topic。
交互：BR418 嵌入 | BR419 布局 | BR420 跳转 | BR847 topicTag。
首页嵌入规则见 §4.4，本节不再重复 home 场景描述。
```
...
[] 全文仅 §4.18 一处维护 Recommended 全局规则（无 §4.7.9 重复）
```
