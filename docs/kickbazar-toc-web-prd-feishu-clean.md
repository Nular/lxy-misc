...
> 版本：**v1.2.3** | 对齐日期：2026/09/23 | 说明：合并 v1.0→v1.2.2 全部确认需求，消除重复块与矛盾表述。  
...
| 2026/09/23 | v1.2.3 | 廖炫尧 | §4.4 改为首页；Recommended 全局定义移至 §4.18                  |
...
| 首页  | Banner + 一级分类 + 特色馆 + Recommended、服务介绍（#7） | P0                                                   |
...
### 4.4 模块 A：首页核心展示区（#6）— v1.2.3 替换原 Recommended 节

**飞书操作：** 删除原「4.4 模块 H：Recommended 推荐组件（#18）」整节，粘贴以下内容。
...
**功能清单：** FL015 Banner · FL016 一级分类 · FL017 特色馆 · FL018 首页 Recommended · FL019 Banner 跳转 · FL020 特色馆跳转 · FL138 View More
...
4. Recommended 召回/展示规则见 **§4.18**（与 Sort By recommend 枚举区分）。
...
|----|------|------|
| BR115 | 首页加载 | 四段结构渲染 |
| BR116 | Banner | 轮播；点击按 linkType 跳转 |
...
**删除整节：** §4.7.9–§4.7.12「商品推荐组件」——与旧 §4.4 重复；全局 Recommended 仅见 §4.18。

---



### 4.18 模块 H：全局推荐组件 Recommended（#18）— v1.2.3 唯一维护点

**飞书操作：** 在 §4.12 或文档末「专题」之后新增本节（编号 4.18）；勿在 §4.4 / §4.7 再写 Recommended 全文。

Recommended 全站复用；`displayMode=recommended` 随机序（与 Sort By 的 recommend 枚举区分）。
...
| pdp | PDP 底栏 | 否 |
| cart | 购物车 | 否 |
| search_empty | 搜索无结果 | 否 |
| store / topic | 店铺 / 专题 | 按宿主页 |

**交互：** BR418 嵌入 · BR419 布局（PC 网格 / Mobile 横滑）· BR420 跳转 PDP · BR847 topicTag

**字段：** 组件配置 + 通用商品卡字段见 v1.0 §4.4.2 / §4.7.11；首页嵌入规则见 §4.4，本节不重复 home 场景描述。
...
- [ ] 结构：Banner → 一级分类 → 特色馆 → Recommended
- [ ] 特色馆四入口→专题页；L1→`/category/{l1Id}`

**全局**

- [ ] 退出登录→`/`
- [ ] topicTag→专题页；Sticky + 回顶
- [ ] Recommended 全局定义仅在 §4.18（无 §4.4/§4.7 重复）
...
| B BR   | 增补 BR115–118/853/854/855/308a；BR851 删除 |
| C 页面对照 | #6 首页→**§4.4**；#18 推荐→**§4.18**（原 §4.4 重复删除）；#17 标本期不做 |
...
| `4.4 模块 H：Recommended`          | **整节删除**，换 §4.4 首页四段结构 |
| `4.7.9`–`4.7.12 商品推荐组件`        | **删除重复节**；保留 §4.18 + §4.7.11 字段 |
