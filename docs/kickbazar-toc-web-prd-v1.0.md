...
| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|---------|
| v1.11 | 2026-09-11 | 产品经理 | **原型对齐迭代**：Header 拆为主/次导航；保障提示条全站全局；搜索改为下拉（Recent + Discovery）；分类抽屉改为 L1/L2/L3+10 推荐三列 Mega Menu；PDP 店铺入口强化（头像）；加购无抽屉+Added successfully 气泡；购物车 Tag/删除/Order Summary 同结算 |
...
| 交易 | 加购成功反馈、购物车页/编辑/Order Summary、结算、结果页、地址弹框 |
| 专题 | 品牌馆、国家馆、精选、潮流 |
...
├── /categories → /category/{id}  分类 Mega Menu + 商品列表
├── /product/{id}  商品详情 + 规格选择器
├── /store/{id}  店铺 + 介绍弹框
├── /cart  购物车页（编辑、Order Summary 同结算；Header 无预览 Dropdown）
...
3. **次导航栏**：**Categories**（带向下箭头 ▾，Hover 展开分类 Mega Menu，详见 §4.6）+ App **一级分类**快捷入口横排；点击一级入口跳转分类落地页，面包屑为 `Home › {一级分类名}`。
...
10. **分类入口（Categories）**：位于**次导航栏**；文案旁展示**向下箭头**；Hover 展开 §4.6 三列 Mega Menu；移出区域收起。  
...
| BR105 | 分类 Mega Menu | Hover Categories（▾） | 展开三列抽屉（BR301） | 加载● | 箭头旋转/高亮 |
...
| 编号 | 需求名称 | 触发条件 | 交互行为 | 状态 | 验收标准 |
|------|---------|---------|---------|------|---------|
| BR830 | 搜索下拉展开 | 点击搜索框 | 锚定搜索框下方；Recent + Discovery | — | 非 Modal |
| BR831 | 搜索下拉收起 | 空白/ESC/路由 | 收起 | — | 不残留 |
...
分类模块与 App 分类树完全一致。用户在次导航 **Hover「Categories ▾」** 时，从次导航下方展开**分类 Mega Menu（Drawer）**。抽屉**自左向右分为三列**，各列可**独立纵向滚动**：

1. **一级分类**：App 一级分类列表  
2. **二级分类**：Hover/选中一级后展示对应二级  
3. **三级分类 + 商品推荐**：选中二级后展示三级列表；同列/右侧展示该分类下**推荐商品，固定展示 10 个**（商品卡同 BR120，点击跳转 PDP）

各列选中态联动：Hover 一级 → 刷新二级；Hover 二级 → 刷新三级与 10 条推荐。

用户亦可从次导航一级入口、首页快捷功能区进入分类落地页 → 面包屑 `Home › {一级名}` → PDP。

#### 功能清单

| 编号 | 功能名称（中文） | 功能名称（英文） | 功能描述 |
|------|----------------|----------------|---------|
| FL036 | Header 分类 Mega Menu | Category Mega Menu | Hover Categories▾ 展开三列抽屉 |
| FL037 | 三级分类联动 | L1/L2/L3 Cascade | 一二三级分类联动展示 |
| FL038 | 分类面包屑 | Category Breadcrumb | 分类落地页层级导航 |
| FL140 | 分类推荐区 | Category Rec Grid | 第三列固定展示 10 个推荐商品 |
...
2. **Header Mega Menu**：次导航 Categories 带 **▾ 箭头**；Hover 展开；移出 Header 次栏+抽屉区域收起；三列各自纵向滚动。  
3. **推荐数量**：第三列商品推荐**固定 10 个**，不分页。  
4. 商品列表排序规则同搜索（BR211–BR213）；分页同 **BR825**（10 行 + View More）。  
5. 面包屑可点击回溯：`Home › 一级`（一级落地页）；多级可扩展。  
...
| 编号 | 需求名称 | 触发条件 | 交互行为 | 状态 | 验收标准 |
|------|---------|---------|---------|------|---------|
| BR301 | 分类 Mega Menu | Hover Categories▾ | 展开三列；各列独立滚动 | 加载● 错误● | L3+10 推荐 |
| BR302 | 分类空态 | 无数据 | 空态 BR137 | 空○ | 可重试 |
| BR303 | 分类联动 | Hover L1/L2 | 联动刷新下级与 10 条推荐 | 加载● 错误● | 推荐数=10 |
...
| 搜索结果 | #9 | FL030–FL033 |
| 搜索排序 | #12 | FL034–FL035 |
| 分类 Mega Menu | #13–#15 | FL036–FL038、FL140 |
