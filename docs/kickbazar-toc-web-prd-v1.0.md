...
| FL134 | 账户下拉菜单 | Account Dropdown Menu | 展示登录/我的订单/我的优惠券等菜单项 |
...
|--------|------|------|--------|--------|---------|
| 登录/注册 | 登录 / 注册 | Sign In | 显示 | 隐藏 | `/login` |
...
5. 菜单项支持 CMS/配置扩展，但本期至少包含上表四项；订单列表/详情/物流/发票页面内容由他人 PRD 承接。  
6. **分类数据**：与 App 一级分类树保持一致；数据未返回前展示骨架屏（同 BR803）。  
7. **滚动阈值**：建议 80px，具体以实现为准，需保证向上滚动可恢复。  
8. **客服入口**：Header 含客服图标（FL132），与购物车、账户并列；点击跳转 `supportEntryUrl`，行为同 **BR806**。  
9. **下拉层级**：账户下拉 `z-index` 低于全屏 Modal/搜索弹窗；打开搜索 Modal 时账户菜单自动收起。
...
| isLoggedIn | 是否已登录 | Is Logged In | boolean | 是 | 当前会话登录态，决定菜单项可见性 |
...
| BR107 | 账户图标 Hover 菜单 | PC Hover 用户图标 | 展开 FL134 下拉菜单；移出收起 | — | 菜单不遮挡搜索框 |
| BR108 | 导航高亮 | 路由匹配 | 高亮当前模块 | — | 切换同步 |
| BR806 | Header 客服入口 | 点击客服图标 | 跳转 `supportEntryUrl` | — | 全站可用；目标页由他人 PRD 定义 |
| BR814 | 账户图标点击菜单 | Mobile/Tablet 点击用户图标 | 展开/收起 FL134 下拉菜单 | — | 再次点击或点空白收起 |
