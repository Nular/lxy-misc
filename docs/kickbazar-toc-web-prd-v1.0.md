...
| BR409 | 资源不存在/下架 | 商品、店铺、专题、订单 |
| BR418–BR420 | 推荐组件 | 首页、PDP、购物车、搜索无结果 |
| BR619 | 登录硬门禁 | 结算、结果页、订单系、地址 |
...
订单模块承接下单后履约查询，**全部页面须登录**（BR640 同 BR619）。包含：五态 Tab 订单列表、订单详情、物流时间轴、发票信息与 PDF 下载。

#### 业务规则

1. **Tab 状态**：全部 / 待付款 / 待发货 / 待收货 / 已完成——与 App 状态机一致。  
2. **待付款**：展示支付倒计时；超时自动刷新状态。  
3. **快捷操作**：按状态展示「去支付」「查看物流」「再次购买」等，操作集同 App。  
...
| BR805 | 图片懒加载 | 商品图 |
| BR137 | 空态（基准） | 见上表 |
| BR138 | Tab 列表（基准） | 店铺 BR504；订单 BR641 |
