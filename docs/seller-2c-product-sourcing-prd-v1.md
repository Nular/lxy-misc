...
| 模块 | 功能点 | 优先级 | 简述 |
|------|--------|--------|------|
| 选品浏览 | 商品列表 | P0 | 展示绑定 2B 货源可售商品（SPU 卡片）；Tab：全部/已发布/未发布 |
...
**功能描述**：以 **SPU 商品卡片** 形式展示绑定 2B 货源的可售商品，支持分页与 **铺货状态 Tab**（全部 / 已发布 / 未发布）；点击卡片进入橱窗型商品详情。
...
3. 列表提供铺货状态 Tab（**SPU 维度**）：
   - **All**：全部可售 SPU
   - **Published**：该 SPU 下所有 SKU 均已上架（**Published**）
   - **Unpublished**：该 SPU 未完全上架，含 **Unpublished** 与 **Partially Published**
