...
const USER_COUPONS = [
  { id: "c_save50", code: "SAVE50", amount: 50, minSpend: 500 },
  { id: "c_save100", code: "SAVE100", amount: 100, minSpend: 1500 },
  { id: "c_vip200", code: "VIP200", amount: 200, minSpend: 3000 },
  { id: "c_beauty", code: "BEAUTY20", amount: 20, minSpend: 200, requiredTags: ["Featured", "Brand Zone"] },
  { id: "c_mega", code: "MEGA500", amount: 500, minSpend: 50000 },
  { id: "c_expired", code: "EXPIRED10", amount: 10, minSpend: 0, simulateExpiredOnConfirm: true },
];
...
const state = {
  ...
  savedAddresses: [],
  drawerCouponSelection: null,
  expiredCouponIds: new Set(),
...
function openModal(html, options = {}) {
  ...
  const { className = "", dismissible = true } = opts;
...
function closeModal(force = false) {
  if (!force && state.modalDismissible === false) return;
...
function closeDrawer() {
  ...
  state.drawerCouponSelection = null;
}
...
function computeOrderSummary(lines) {
  ...
  const coupon = state.appliedCoupon?.amount || 0;
  const shipping = 60;
  const codFee = 0;
...
function getOrderContext() {
  const lines = getSelectedCartLines();
  const merchandise = lines.reduce((s, l) => s + l.price * l.qty, 0);
  const tags = new Set(lines.map((l) => l.tag).filter(Boolean));
  return { merchandise, tags, lines };
}

function evaluateCoupon(coupon) {
  if (state.expiredCouponIds.has(coupon.id)) {
    return { status: "unavailable", reason: "Coupon expired" };
  }
  const { merchandise, tags } = getOrderContext();
  if (merchandise < coupon.minSpend) {
    return { status: "unavailable", reason: "Minimum spend not met" };
  }
  if (coupon.requiredTags?.length) {
    const ok = coupon.requiredTags.some((t) => tags.has(t));
    if (!ok) return { status: "unavailable", reason: "Not applicable to current items" };
  }
  return { status: "available" };
}

function getCouponsWithStatus() {
  return USER_COUPONS.map((c) => ({ ...c, ...evaluateCoupon(c) }));
}

function pickBestAvailableCouponId() {
  const available = getCouponsWithStatus().filter((c) => c.status === "available");
  if (!available.length) return null;
  return available.reduce((best, c) => (c.amount > best.amount ? c : best)).id;
}

function validateAddressForm(data) {
  const errors = [];
  if (!data.recipientName?.trim()) errors.push("Name is required");
  if (!data.phone?.trim()) errors.push("Phone is required");
  else if (!/^01[0-9]{9}$/.test(data.phone.replace(/\s/g, ""))) errors.push("Invalid Bangladesh phone number");
  if (!data.regionCityDistrict?.trim()) errors.push("Region / City / District is required");
  if (!data.addressLine?.trim()) errors.push("Address is required");
  return errors;
}

function saveShippingAddress(data) {
  const addressId = data.addressId || `addr_${Date.now()}`;
  const saved = {
    addressId,
    recipientName: data.recipientName.trim(),
    phone: data.phone.trim(),
    regionCityDistrict: data.regionCityDistrict.trim(),
    addressLine: data.addressLine.trim(),
    isDefault: true,
  };
  state.shippingAddress = saved;
  const idx = state.savedAddresses.findIndex((a) => a.addressId === addressId);
  if (idx >= 0) state.savedAddresses[idx] = saved;
  else state.savedAddresses.unshift(saved);
  return saved;
}

function addressFormHTML(values = {}, { title = "Add Shipping Address", showClose = true } = {}) {
  const v = {
    recipientName: values.recipientName || "",
    phone: values.phone || "",
    regionCityDistrict: values.regionCityDistrict || "",
    addressLine: values.addressLine || "",
  };
  return `
    <div class="modal__head">
      <h2>${title}</h2>
      ${showClose ? `<button type="button" class="modal__close" data-close-modal>×</button>` : ""}
    </div>
    <p style="font-size:13px;color:#666;margin:0 0 16px">Fill in your delivery address. It will be saved to your account.</p>
    <div class="form-grid">
      <div class="form-field"><label>Name</label><input id="addr-name" value="${v.recipientName}"></div>
      <div class="form-field"><label>Phone (+880)</label><input id="addr-phone" value="${v.phone}" placeholder="01XXXXXXXXX"></div>
      <div class="form-field full"><label>Region / City / District</label><input id="addr-region" value="${v.regionCityDistrict}"></div>
      <div class="form-field full"><label>Address</label><textarea id="addr-line" rows="2">${v.addressLine}</textarea></div>
    </div>
    <div class="modal__foot">
      <button type="button" class="btn btn--primary btn--block" id="addr-save">Save</button>
    </div>`;
}

function bindAddressFormSave(onSaved) {
  document.getElementById("addr-save").onclick = () => {
    const data = {
      addressId: state.shippingAddress?.addressId,
      recipientName: document.getElementById("addr-name").value,
      phone: document.getElementById("addr-phone").value,
      regionCityDistrict: document.getElementById("addr-region").value,
...
function openAddressModal({ required = false, edit = false } = {}) {
  const values = edit && state.shippingAddress ? state.shippingAddress : {};
  openModal(
    addressFormHTML(values, {
      title: required ? "Add Shipping Address" : edit ? "Edit Address" : "Add Address",
      showClose: !required,
    }),
    { dismissible: !required, className: "wide" }
  );
  bindAddressFormSave();
}
...
function openSavedAddressesModal() {
  const list =
    state.savedAddresses.length > 0
      ? state.savedAddresses
          .map(
            (a) => `
        <button type="button" class="saved-address-item${state.shippingAddress?.addressId === a.addressId ? " active" : ""}" data-pick-addr="${a.addressId}">
          <strong>${a.recipientName}</strong> · ${a.phone}<br>
          <span style="font-size:13px;color:#666">${a.regionCityDistrict}<br>${a.addressLine}</span>
        </button>`
          )
          .join("")
  ...
  openModal(
    ...
    ${list}
    <button type="button" class="btn btn--block" id="addr-add-new" style="margin-top:12px">Add New Address</button>`,
    "wide"
  );

  document.querySelectorAll("[data-pick-addr]").forEach((btn) => {
    btn.onclick = () => {
      const addr = state.savedAddresses.find((a) => a.addressId === btn.dataset.pickAddr);
      if (addr) {
        state.shippingAddress = { ...addr };
        closeModal(true);
        render();
      }
    };
  });
  document.getElementById("addr-add-new").onclick = () => {
    closeModal(true);
    openAddressModal();
  };
}
...
function renderShippingAddressBlock() {
  if (!state.shippingAddress) {
    return `
      <div class="address-empty">
        <p>No shipping address yet. Please add your delivery address to continue.</p>
  ...
  return `
    <div class="address-card">
      <p class="address-card__name">${a.recipientName} · ${a.phone}</p>
      <p class="address-card__region">${a.regionCityDistrict}</p>
      <p class="address-card__line">${a.addressLine}</p>
    </div>
...
function couponCardHTML(coupon, { selectable = true } = {}) {
  const disabled = coupon.status === "unavailable";
  const selected = selectable && state.drawerCouponSelection === coupon.id;
  return `
    <button type="button"
      class="coupon-card${selected ? " selected" : ""}${disabled ? " coupon-card--disabled" : ""}"
      data-coupon-id="${coupon.id}"
      ${disabled || !selectable ? "disabled" : ""}>
      <div class="coupon-card__amount">${formatBDT(coupon.amount)} OFF</div>
      <div class="coupon-card__code">${coupon.code}</div>
      <div class="coupon-card__meta">Min. spend ${formatBDT(coupon.minSpend)}</div>
      ${disabled ? `<div class="coupon-card__reason">${coupon.reason}</div>` : ""}
    </button>`;
}

function renderCouponDrawerContent() {
  const coupons = getCouponsWithStatus();
  const available = coupons.filter((c) => c.status === "available");
  const unavailable = coupons.filter((c) => c.status === "unavailable");

  openDrawer(`
    <div class="drawer__head">
      <h2>My Coupons</h2>
      <button type="button" class="modal__close" data-close-drawer aria-label="Close">×</button>
    </div>
    <div class="drawer__body">
      <div class="coupon-section">
        <p class="coupon-section__title">Available</p>
        ${available.length ? available.map((c) => couponCardHTML(c)).join("") : `<p style="color:#888;font-size:13px">No available coupons for this order.</p>`}
      </div>
      <div class="coupon-section">
        <p class="coupon-section__title">Not Available</p>
        ${unavailable.length ? unavailable.map((c) => couponCardHTML(c, { selectable: false })).join("") : `<p style="color:#888;font-size:13px">None</p>`}
      </div>
    </div>
    <div class="drawer__foot">
      <button type="button" class="btn" data-close-drawer>Close</button>
      <button type="button" class="btn btn--primary" id="coupon-confirm">Confirm</button>
    </div>`);

  document.querySelectorAll(".coupon-card:not(.coupon-card--disabled)").forEach((btn) => {
    btn.onclick = () => {
      const id = btn.dataset.couponId;
      state.drawerCouponSelection = state.drawerCouponSelection === id ? null : id;
      renderCouponDrawerContent();
    };
  });
  ...
  document.getElementById("coupon-confirm").onclick = () => confirmCouponSelection();
}

function openCouponDrawer() {
  if (state.appliedCoupon?.id && evaluateCoupon(USER_COUPONS.find((c) => c.id === state.appliedCoupon.id)).status === "available") {
    state.drawerCouponSelection = state.appliedCoupon.id;
  } else {
    state.drawerCouponSelection = pickBestAvailableCouponId();
  }
  renderCouponDrawerContent();
}

function confirmCouponSelection() {
  if (!state.drawerCouponSelection) {
    state.appliedCoupon = null;
    closeDrawer();
    render();
    return;
  }
  const coupon = USER_COUPONS.find((c) => c.id === state.drawerCouponSelection);
  if (!coupon) return;

  if (coupon.simulateExpiredOnConfirm) {
    state.expiredCouponIds.add(coupon.id);
    showToast("Coupon expired or invalid. Please select another.");
    state.drawerCouponSelection = pickBestAvailableCouponId();
    renderCouponDrawerContent();
    return;
  }

  const ev = evaluateCoupon(coupon);
  if (ev.status !== "available") {
    showToast(ev.reason || "Coupon unavailable");
    renderCouponDrawerContent();
    return;
  }

  state.appliedCoupon = { id: coupon.id, code: coupon.code, amount: coupon.amount };
  closeDrawer();
...
function applyCouponCode(rawCode) {
  ...
  const coupon = USER_COUPONS.find((c) => c.code === code);
  if (!coupon) {
    showToast("Invalid coupon code");
    return;
  }
  if (state.expiredCouponIds.has(coupon.id)) {
    showToast("Coupon expired or invalid");
    return;
  }
  const ev = evaluateCoupon(coupon);
  if (ev.status !== "available") {
...
function bindCheckoutPageEvents() {
  document.getElementById("add-address")?.addEventListener("click", () => openAddressModal({ required: true }));
  document.getElementById("change-addr")?.addEventListener("click", () => openSavedAddressesModal());

  document.getElementById("apply-coupon-code")?.addEventListener("click", () => {
    applyCouponCode(document.getElementById("coupon-code-input").value);
  });
  document.getElementById("coupon-code-input")?.addEventListener("keydown", (e) => {
    if (e.key === "Enter") applyCouponCode(e.target.value);
  });

  document.getElementById("open-coupons")?.addEventListener("click", openCouponDrawer);

  document.getElementById("place-order")?.addEventListener("click", () => {
    if (!state.shippingAddress) {
      showToast("Please add a shipping address");
      openAddressModal({ required: true });
      return;
    }
...
function renderCheckout() {
  ...
  if (state.appliedCoupon) {
    const couponDef = USER_COUPONS.find((c) => c.id === state.appliedCoupon.id);
    if (!couponDef || evaluateCoupon(couponDef).status !== "available") {
      state.appliedCoupon = null;
    }
  }

  const orderItemsHTML = selectedLines.length
    ? selectedLines
        .map((l) => `<p style="font-size:13px;margin:0 0 8px"><strong>${l.store || "Seller"}</strong> — ${l.title} × ${l.qty}</p>`)
        .join("")
    : `<p style="font-size:13px;color:#888">No items selected. <a href="#/cart" data-nav>Return to cart</a></p>`;
  ...
  document.getElementById("app").innerHTML = `
    ...
    <div class="checkout-layout">
      <div>
        ...
        <div class="form-block"><h3>Order Items</h3>${orderItemsHTML}</div>
        <div class="form-block"><h3>Payment</h3><div class="payment-option"><input type="radio" checked disabled> Cash on Delivery (COD)</div></div>
      </div>
      <div class="summary-card">
        <h3 style="margin:0 0 12px">Coupon & Code</h3>
        ${
          state.appliedCoupon
            ? `<p class="coupon-applied">Applied: <strong>${state.appliedCoupon.code}</strong> (−${formatBDT(state.appliedCoupon.amount)})</p>`
            : ""
        }
        <div class="coupon-row">
          <input id="coupon-code-input" placeholder="Enter coupon code" value="${state.appliedCoupon?.code || ""}">
          <button type="button" class="btn" id="apply-coupon-code">Apply</button>
        </div>
  ...
  if (!state.shippingAddress) {
    setTimeout(() => openAddressModal({ required: true }), 0);
  }
}
...
function initEvents() {
  ...
  document.getElementById("modal").addEventListener("click", (e) => {
    if (state.modalDismissible === false) return;
    if (e.target.matches("[data-close-modal]") || e.target.classList.contains("modal__backdrop")) closeModal(true);
  });

  document.getElementById("drawer").addEventListener("click", (e) => {
    if (e.target.matches("[data-close-drawer]") || e.target.classList.contains("drawer__backdrop")) closeDrawer();
  });
